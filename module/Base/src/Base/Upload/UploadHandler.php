...
class UploadHandler {
    ...
    public function getName(){
        return $this->getFullRandomName();
    }

    private function getFullRandomName(){
        $nome = @$_SESSION['Admin']['noticiaprefixoid'];
        if(empty($nome))
            $nome = time();
        $nome .= $_REQUEST['qquuid'].sprintf("%05d",rand()).'.';
        if (isset($_REQUEST['qqfilename'])){
            $pathinfo = pathinfo($_REQUEST['qqfilename']);
            $nome.= isset($pathinfo['extension']) ? $pathinfo['extension'] : '';
        ...
        # non-chunked upload
            ...
            if ($target){
                ...
                if (move_uploaded_file($file['tmp_name'], $target)){
                    ...
                    $imagem = $target;
                    // Redimensionando imagem para padrão
                    $fotoLargura=1920;
                    $fotoAltura=1200;
                    $imagemInfo = getimagesize($imagem);
                    $imagemInfo[2] = $imageTypeArray[$imagemInfo[2]];
                    list($imagemOriginalLargura,$imagemOriginalAltura) = $imagemInfo;
                    $imagemAltura = $imagemOriginalAltura;
                    $imagemLargura = $imagemOriginalLargura;
                    ...
                    $imgPP = imagecreatefromstring(file_get_contents($imagem));
                    ...
                    if($imagemInfo[2]=='JPG'){
                        imagejpeg($imgP, $imagem, $fotoQualidade);
                    }else if($imagemInfo[2]=='GIF'){
                        imagegif($imgP, $imagem);
                    }else{
                        imagepng($imgP, $imagem, (100-$fotoQualidade)/10);
                    }
    ...
    /**
     * Remove a file terminate with uuid in a imgsDirectory defaul
     * @param string $dir
     */
    protected function removeFile($dir, $uuid){
        $ereg = "/^.+".$uuid."[0-9]{5}\.[a-z0-9]{2,6}$/";
       ...
       foreach($arquivos as $imgnome){
            if(preg_match($ereg,$imgnome)){
                $arquivo = join(DIRECTORY_SEPARATOR, array($dir, $imgnome));
                if(is_file($arquivo)){
                    unlink($arquivo);
                }
            }
